...
const submit = () => {
    if (editingId.value) {
        form.put(`/admin/kelola-user/${editingId.value}`, { onSuccess: close, preserveScroll: true });
    } else {
        form.post('/admin/kelola-user', { onSuccess: close, preserveScroll: true });
    }
};
const destroy = (id) => { if (confirm('Hapus user ini?')) router.delete(`/admin/kelola-user/${id}`, { preserveScroll: true }); };
    ...
    <div class="px-6 py-8 lg:px-10">
        ...
        <div v-if="showForm" class="mb-6 rounded-2xl border border-orange-200 bg-orange-50/30 p-6">
            <div class="flex items-center justify-between mb-4">
                <h2 class="text-sm font-bold text-slate-800">{{ editingId ? 'Edit User' : 'Tambah User Baru' }}</h2>
                <button @click="close" class="text-slate-400 hover:text-slate-600"><X class="h-4 w-4" /></button>
            </div>
            <form @submit.prevent="submit" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div><label class="text-xs font-semibold text-slate-600">Nama</label><input v-model="form.name" class="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500" /></div>
                <div><label class="text-xs font-semibold text-slate-600">Email</label><input v-model="form.email" type="email" class="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500" /></div>
                <div><label class="text-xs font-semibold text-slate-600">Password {{ editingId ? '(kosongkan jika tidak diubah)' : '' }}</label><input v-model="form.password" type="password" class="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500" /></div>
                <div><label class="text-xs font-semibold text-slate-600">Role</label>
                    <select v-model="form.role" class="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500">
            ...
            <table class="w-full text-sm">
                <thead class="border-b border-slate-100 bg-slate-50/80">
                    <tr>
                        ...
                        <th class="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">Bergabung</th>
                    ...
                    <tr v-for="u in users" :key="u.id" class="hover:bg-slate-50/60 transition-colors">
                        ...
                        <td class="px-5 py-3.5 text-center">
                            <div class="flex items-center justify-center gap-2">
                                <button @click="openEdit(u)" class="rounded-lg p-1.5 text-slate-400 hover:bg-orange-50 hover:text-orange-600 transition-colors"><Pencil class="h-4 w-4" /></button>
                                <button @click="destroy(u.id)" class="rounded-lg p-1.5 text-slate-400 hover:bg-red-50 hover:text-red-600 transition-colors"><Trash2 class="h-4 w-4" /></button>
                            </div>
                        </td>
                    </tr>
