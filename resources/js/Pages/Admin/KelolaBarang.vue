...
import { Package, Plus, Pencil, Trash2, X, Image as ImageIcon } from '@lucide/vue';
...
const openEdit = (b) => {
    ...
    imagePreview.value = null;
    existingImage.value = b.image_path || null;
    showForm.value = true;
};
const close = () => { 
                    ...
                    <tr v-for="b in barangs" :key="b.id" class="hover:bg-slate-50/60 transition-colors">
                        ...
                        <td class="px-5 py-3.5">
                            <div class="h-10 w-16 overflow-hidden rounded-lg border border-slate-100 bg-slate-50">
                                <img v-if="b.image_path" :src="b.image_path" :alt="b.nama" class="h-full w-full object-cover" />
                        ...
                        <td class="px-5 py-3.5 text-center">
                            <div class="flex items-center justify-center gap-2">
                                <button @click="openEdit(b)" class="rounded-lg p-1.5 text-slate-400 hover:bg-orange-50 hover:text-orange-600 transition-colors" title="Edit"><Pencil class="h-4 w-4" /></button>
                                <button @click="destroy(b.id)" class="rounded-lg p-1.5 text-slate-400 hover:bg-red-50 hover:text-red-600 transition-colors" title="Hapus"><Trash2 class="h-4 w-4" /></button>
                            </div>
                        </td>
                    </tr>
                    <tr v-if="!barangs.length"><td colspan="7" class="px-5 py-12 text-center text-slate-400">Belum ada data barang.</td></tr>
                </tbody>
            </table>
        </div>
    </div>
